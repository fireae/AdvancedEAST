...
def predict(east_detect, img_path, pixel_threshold, quiet=False):
    ...
    with Image.open(img_path) as im:
        ...
        scale_ratio_w = 1.0*d_wight / im.width
        scale_ratio_h = 1.0*d_height / im.height
...
def predict_tf(sess, img_path, pixel_threshold, quiet=False):
    img = image.load_img(img_path)
    d_wight, d_height = resize_image(img, cfg.max_predict_img_size)
    img = img.resize((d_wight, d_height), Image.NEAREST).convert('RGB')
    img = image.img_to_array(img)
    ...
    x = np.expand_dims(img, axis=0)
    # y = east_detect.predict(x)
    input = sess.graph.get_tensor_by_name('input_img:0')
    output = sess.graph.get_tensor_by_name('east_detect/concat:0')
    y = sess.run([output], feed_dict={'input_img:0':x})
    print(y)
    y = np.squeeze(y, axis=0)
    y[:, :, :3] = sigmoid(y[:, :, :3])
    cond = np.greater_equal(y[:, :, 0], pixel_threshold)
...
if __name__ == '__main__':
    ...
    #
    # east = East()
    # east_detect = east.build_network()
    # # east_detect.load_weights(cfg.saved_model_weights_file_path)
    # # east_detect.save_weights('./east_port/east', save_format='tf')
    # east_detect.load_weights('./east_port/east')
    # print(east_detect.input.name)
    # print(east_detect.output.name)
    # save_model_for_production(east_detect, "1")
## =======================================================================

    export_dir = 'prod_models/1/'
    with tf.Session(graph=tf.Graph()) as sess:
        tf.saved_model.loader.load(sess, [tf.saved_model.tag_constants.SERVING], export_dir)
        input = sess.graph.get_tensor_by_name('input_img:0')
        output = sess.graph.get_tensor_by_name('east_detect/concat:0')
        predict_tf(sess, img_path, threshold)